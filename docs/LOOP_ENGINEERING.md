...
7. For every implementation task that has an open pull request and `review_status: none`, set `review_status: pending`.
8. Identify tasks with `status: Ready` whose dependencies are complete.
9. Treat a dependency as complete only when its task is `Done` and its `pr_status` is `merged`, unless the task is archived in `LOOP_LOG.md` or was explicitly completed before the branch/PR rule and has `pr_status: none`.
10. Treat review work as eligible when the implementation PR is open and `review_status` is `pending` or `changes_requested`.
11. Group ready tasks by file/scope.
12. Select one or more independent tasks.
13. Move each selected task to `In Progress`, set `owner`, and set `started`.
14. Ensure the task has a branch name. If it does not, add one before launch.
15. Create or switch to the task branch before implementation tasks.
16. Launch multiple executor subagents at the same time when there are independent ready tasks; do not assign two agents to the same files/scope.
17. Keep one task local if it involves coordination, PM scoping, environment setup, GitHub setup, or state-file updates.
18. When implementation work finishes, push the task branch and open a GitHub pull request.
19. Record the pull request URL, `pr_status: open`, and `pr_last_checked` in `LOOP_TASKS.md`.
20. For each new implementation PR, set `review_status: pending`.
21. Move tasks to `Done` or `Blocked`.
22. Archive fully complete tasks into `LOOP_LOG.md` to keep `LOOP_TASKS.md` small.
23. Record checks, PR statuses, review statuses, newly planned tasks, archived tasks, PM decisions, failures, and next actions in `LOOP_STATE.md`.
