...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T040 | Blocked | coordinator | 2026-06-28 21:59 UTC+2 | task/T040-run-billa-real-data-ingest |  | none |  | none |  | Run approved BILLA real-data ingest, normalize, and report counts | Operational run plus `LOOP_STATE.md`; no code changes expected unless the run exposes a bug | T021,T022,T008,T009 | Highest priority for real data. Blocked only on explicit human choice of scope: capped validation or broad BILLA run. Acceptance: run approved `scripts/billa_full_ingest.py --store` scope, record `scrape_run_id`, run sanity report, run normalization into `retailer_products`, report raw/normalized counts and data-quality issues, and stop if runbook stop conditions appear. Do not run MPREIS storage here. |
