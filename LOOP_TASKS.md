...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T010 | In Progress | implementation-subagent | 2026-06-28 21:13 UTC+2 | task/T010-billa-product-search-api |  | none |  | none |  | Add BILLA-only product search API | `backend/app/api`, `backend/app/db`, `backend/app/main.py`, API tests | T008 | Ready after T008 merged; acceptance: `GET /products/search?q=...` returns normalized BILLA products with price/unit/category/source fields. |
...
| T021 | In Progress | implementation-subagent | 2026-06-28 21:13 UTC+2 | task/T021-billa-controlled-full-ingest |  | none |  | in_progress | self-review | Add controlled BILLA full-ingest workflow | `scripts`, `README.md`, optional read-only quality/report integration | T009 | Ready after T009 merged; acceptance: operator-triggered BILLA ingest with explicit broad-run confirmation, rate limiting, resume/limit options, raw storage, run summary, and sanity-report output. |
...
| T031 | Done | coordinator | 2026-06-28 21:13 UTC+2 | task/T031-md-only-coordinator-review-skip |  | none |  | passed | review-subagent | Exempt Markdown-only coordinator PRs from review gate | `.github/workflows/agent-review.yml`, `scripts/check_pr_review_status.py`, `docs/LOOP_ENGINEERING.md`, `LOOP_TASKS.md`, tests |  | Review passed. CI/CD now exempts Markdown-only coordinator PRs while non-Markdown PRs still require `review_status: passed`. |
