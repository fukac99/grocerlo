...
| T003 | Blocked | postgres-store-subagent | 2026-06-28 19:59 UTC+2 | task/T003-postgres-store-path | https://github.com/fukac99/grocerlo/pull/11 | open | 2026-06-28 20:03 UTC+2 | changes_requested | review-subagent | Start Postgres, run migrations, and test `--store` path | `docker-compose.yml`, `backend/alembic`, database | T001,T002,T006 | Changes requested: Docker daemon was unavailable, so Postgres startup, migrations, and `scripts/scrape_once.py --store` were not run. Low-volume dry BILLA scrape passed for 1 category and 3 products. T008/T009 remain blocked until stored validation succeeds. |
...
| T012 | Ready |  |  | task/T012-retailer-discovery-checklist |  | none |  | none |  | Document retailer expansion discovery checklist | `docs`, maybe `PRICE_COMPARISON_APP_PLAN.md` only | T002,T005 | T002 and T005 have merged; acceptance: checklist for MPREIS/REWE/Kaufland/Tesco discovery covering price availability, location dependence, promotions/app-only flags, source IDs, robots/terms notes, low-volume limits. |
...
| T014 | Done | coordinator | 2026-06-28 19:34 UTC+2 | task/T014-loop-ledger-updates | https://github.com/fukac99/grocerlo/pull/8 | open | 2026-06-28 19:43 UTC+2 | passed | review-subagent | Update loop ledger after executor reviews and PM scoping | `LOOP_TASKS.md`, `LOOP_STATE.md` | T002,T005,T007 | Review passed. Fixed PR branch gate compatibility and blocked downstream tasks until PR-backed dependencies merge. |
