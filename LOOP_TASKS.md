...
| T003 | Blocked |  |  | task/T003-postgres-store-path |  | none |  | none |  | Start Postgres, run migrations, and test `--store` path | `docker-compose.yml`, `backend/alembic`, database | T001,T002,T006 | Blocked until T002 PR merges; only after dry scrape output looks plausible. |
...
| T012 | Blocked |  |  | task/T012-retailer-discovery-checklist |  | none |  | none |  | Document retailer expansion discovery checklist | `docs`, maybe `PRICE_COMPARISON_APP_PLAN.md` only | T002,T005 | Blocked until T002 and T005 PRs merge; acceptance: checklist for MPREIS/REWE/Kaufland/Tesco discovery covering price availability, location dependence, promotions/app-only flags, source IDs, robots/terms notes, low-volume limits. |
